...
class FreeRoamEnemy {
	...
	update(deltaTime) {
		if (this.mode === "chase") {
			...
			if (currentTime - this.lastChaseTargetUpdate > this.chaseTargetUpdateInterval) {
				this.targetX = player.x + player.width / 2 - this.width / 2;
				this.targetY = player.y + player.height / 2 - this.height / 2;
				this.lastChaseTargetUpdate = currentTime;
			}
		}
...
class EliteEnemy extends BaseEliteEnemy {
	...
	shoot() {
		const bulletX = this.x + this.width / 2 - 15 / 2;
...
class BarrageEnemy extends BaseEliteEnemy {
	...
	generateExplosionBullets(x, y) {
		...
		for (let i = 0; i < numBullets; i++) {
			...
			explosionBullets.push(new GenericEnemyBullet(x, y, targetX, targetY, 8, 8, 5 * 60, COLORS.YELLOW, 8)); // 秒速に
		}
	}
...
function getAdjustedValue(baseValue, level, reductionRate = 0.9, increaseRate = 1.1) {
	const power = level - 1;
	// baseValueはクールダウン(ms)か速度(px/sec)のどちらかなので、条件を変更
	if (baseValue >= 1000) {
		// Cooldowns, intervals
		return baseValue * Math.pow(reductionRate, power);
	} else {
		// Speeds, etc.
		return baseValue * Math.pow(increaseRate, power);
	}
}
...
function update(deltaTime) {
	...
	bullets.forEach((b) => {
		...
		else b.y -= bulletSettings.speed * deltaTime; // 直進弾もdeltaTime適用
	});
	...
	if (currentTime - lastFreeroamSpawnTime > currentFreeroamSpawnInterval) {
		const spawnX = Math.random() < 0.5 ? -20 : SCREEN_WIDTH + 20;
		const spawnY = Math.random() * (SCREEN_HEIGHT / 3);
	...
	freeRoamEnemies = freeRoamEnemies.filter(
		(e) => e.y < SCREEN_HEIGHT + e.height && e.x > -e.width && e.x < SCREEN_WIDTH + e.width
	);
	...
	if (player.hp <= 0) {
		console.log("ゲームオーバー！ プレイヤーのHPが0になりました。");
		gameOver = true;
	}
}
...
function draw() {
	...
	if (player.shields > 0) {
		player.shieldOffsetAngle =
			(player.shieldOffsetAngle + 200 * (lastTime ? (performance.now() - lastTime) / 1000 : 0)) % 360;
